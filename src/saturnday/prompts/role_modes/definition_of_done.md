...
### 7. Design reasoning truth
Code that works but shows no design decisions is not senior-quality complete.
Check for:
- Comments explaining WHY (not WHAT) for non-obvious choices
- Trade-off decisions visible in comments or docstrings
- Test effort concentrated on the risky/complex parts, not uniformly distributed
- LICENSE file present in the repository
If the code is uniformly polished with no visible design thinking, note that as a quality gap.

### 8. Project runnable truth
A project is NOT complete unless it can actually be executed.
Check for:
- TypeScript/JavaScript: package.json with dependencies and scripts, tsconfig.json for TS
- Python: pyproject.toml or setup.py with dependencies and entry points
- README explains how to install and run
- If the project cannot be installed and executed from a clean checkout, it is NOT DONE
A repository of source files with no build/run config is incomplete work, not a delivered project.

### 9. Doc-code consistency truth
Documentation must match the actual code behaviour.
Check for:
- README install instructions match actual project language and setup
- SKILL.md behaviour claims match actual function behaviour (e.g. if docs say "prints to stderr", verify code actually uses stderr)
- Feature lists in docs match features actually registered/wired in the code
- Version/compatibility claims match actual syntax used
- If docs say the tool does 5 things, verify all 5 are wired into the main execution path
Flag any mismatch as a completion blocker.

### 10. Brief completeness truth
If the original brief is provided, verify that every capability, integration, or feature mentioned in the brief has a working implementation in the code.

Check for:
- If the brief says "connect to X" or "ingest from X", verify actual API clients or connectors exist for each service — not just data structures or normalisation functions that expect raw data to be supplied
- If the brief mentions external services (Slack, GitHub, email, databases), verify that credentials are handled via environment variables and that the code can actually reach those services when configured
- A skill that normalises data but cannot acquire it does not fulfil a brief that says "ingest from" those sources

If the brief is not available, skip this check.

### 11. Usability truth
The project must be usable, not just technically correct.
Check for:
- If this is an OpenClaw skill (has SKILL.md): verify SKILL.md contains operational instructions telling the agent how and when to invoke the skill, what inputs it accepts, and what outputs it produces. A SKILL.md that only describes what the skill does without explaining how to use it is incomplete.
- If the project has package.json or pyproject.toml: verify it has an entry point (bin, main, start script, or console_scripts) so it can actually be run
- If the project exports functions but has no CLI wrapper, API endpoint, or documented programmatic usage: it is not usable
- Code that passes all tests but cannot be invoked by a user or agent is NOT complete
