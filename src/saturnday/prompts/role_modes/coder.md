...
> **Architecture note:** The canonical coder system prompt is assembled by
> `context_assembler.build_system_prompt()` from `coder_system_senior_v1.txt`
> plus the engineering standards corpus. This role-mode file provides the
> Saturnday-specific coder persona for auxiliary role passes via `invoke_role`.
> For primary ticket execution, the context_assembler path is canonical.
